...
train_label_array_path = '/home/kumarv/pravirat/Realsat_labelling/UnifiedModel/Train_store.npy'
load_train_label_array_path = '/home/kumarv/pravirat/Realsat_labelling/UnifiedModel/Train.npy'
...
test_load = np.load(load_test_label_array_path)
testlist = test_label_array.tolist()
# exit()
# trainlist=====================================
train_label_array = np.load(train_label_array_path)
train_load = np.load(load_train_label_array_path)
trainlist = train_label_array.tolist()
...
for i, d in enumerate(trainlist):
    ...
    if d == 3:
        TrainOther.append(i)
TrainOther.remove(TrainOther[-1])
...
continent_ID_list = [x for x in continent_ID_list if x not in TrainFarm]
continent_ID_list = [x for x in continent_ID_list if x not in TrainRiver]
continent_ID_list = [x for x in continent_ID_list if x not in TestFarm]
continent_ID_list = [x for x in continent_ID_list if x not in TestRiver]
TrainOther = [x for x in continent_ID_list if x not in TestOther]

# Data loader===============================================================================================
FinalIDs_train = []
FinalIDs_train.extend(TrainFarm)
FinalIDs_train.extend(TrainRiver)
TrainOther = TestOther[:(len(TrainFarm)-len(TrainRiver))]
FinalIDs_train.extend(TrainOther)
...
for ID in FinalIDs_train:
    if(len(frac_map_images_list_train)%1000==0):
        print(len(frac_map_images_list_train))
    frac_map_image, label_image, ID = create_frac_map_and_label(ID,train_load)            
...
# TestOther = TestOther[:(len(TestFarm)-len(TestRiver))]
...
for ID in FinalIDs_test:
    ...
    frac_map_image, label_image, ID = create_frac_map_and_label(ID,test_load)            
...
while(ACC<0.9):
    no_epochs += 50
    experiment_id +=1
    if(len(ACC_ratio_list)>=3):
        last1 = ACC_ratio_list[-1]
        last2 = ACC_ratio_list[-2]
        last3 = ACC_ratio_list[-3]
    ...
    for epoch in range(1,no_epochs+1):
        ...
        if(epoch%100 == 0):
            ...
            labels = []
            IDs_all = []
            totallist1= test_label_array.tolist()
            ...
            print(counting1,"FARM ACCURACY: ")
            farm_overlap = [x for x in TestFarm if x in farm_prediction]
            print(len(farm_overlap)/len(TestFarm))

            print(counting2,"RIVER ACCURACY: ")
            river_overlap = [x for x in TestRiver if x in river_prediction]
            print(len(river_overlap)/len(TestRiver))
            
            ACC = (len(farm_overlap)+len(river_overlap))/(len(TestFarm)+len(TestRiver))
